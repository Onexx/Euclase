...
ObservableRect::ObservableRect(const QRectF& rect, QGraphicsItem* parent)
    ...
    str += "/Materials/BtnSounds/Stand.wav";
    sound = new QSound(str);
}
...
void ObservableRect::mouseReleaseEvent(QGraphicsSceneMouseEvent* event) {
    ...
    if (event->scenePos() == previousPosition &&
        ...
        QString FileName = "Materials/Description/";
        ...
        msgBox->setButtonText(QMessageBox::Ok, Data::navBtns::print);
        msgBox->setButtonText(QMessageBox::Cancel, Data::navBtns::cancel);
        int n = msgBox->exec();
